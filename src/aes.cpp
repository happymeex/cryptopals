...
block inverseMixColumns(const block &b) {
    ...
    for (int i = 0; i < 16; i++) {
        int rem = i % 4;
        int val = mul14[b.at(i)] ^ mul9[b.at(i - 1 + ((rem == 0) ? 4 : 0))] ^
                  mul13[b.at(i - 2 + ((rem <= 1) ? 4 : 0))] ^
                  mul11[b.at(i - 3 + ((rem <= 2) ? 4 : 0))];
        ret[i] = val;
    }
    return ret;
}
...
std::array<int, 4> keyScheduleCore(const std::array<int, 4> &in,
                                   int iteration) {
    std::array<int, 4> ret;
 ...
 *
 */
block invRound(block b, const block &key, bool diffusion = true,
               bool mixColumns = true) {
    block ret = b;
    blockXor(ret, key);
    if (diffusion) {
        if (mixColumns)
            ret = inverseMixColumns(ret);
        ret = shiftRows(ret, -1);
        ret = invSubBytes(ret);
    }
    return ret;
...
ByteSeq aes128_ecb_decrypt(const ByteSeq &cipher, const std::string &key) {
    ...
    for (int round = 10; round >= 0; round--) {
        for (int i = 0; i < blocks.size(); i++) {
            blocks[i] =
                invRound(blocks[i], keys[round], round != 0, round != 10);
        }
    }
